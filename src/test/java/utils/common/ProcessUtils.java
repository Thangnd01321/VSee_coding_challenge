...
public class ProcessUtils {
  ...
  public static void startRecording(String bashScript, String filePath)
    ...
    if (System.getProperty("os.name").startsWith("Mac")) {
      command =
          "/usr/local/bin/ffmpeg -f avfoundation -i 0 -pix_fmt yuv420p -vcodec mpeg4 -r 3 "
              + filePath;
      ...
      PrintWriter writer =
          new PrintWriter(
              Paths.get(TestConfig.TOOL_PATH, "startRecording.bat").toString(), "UTF-8");
      writer.println(command);
      writer.close();
      ...
      command = Paths.get(TestConfig.TOOL_PATH, "startRecording.bat").toString();
    }
    startProcess(command);
  }
  ...
  public static void killAllAppiumNode() throws IOException, InterruptedException {
    String command = "";
    if (System.getProperty("os.name").startsWith("Mac")) {
